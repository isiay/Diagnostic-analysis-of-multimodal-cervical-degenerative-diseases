...
class MultiTaskLoss(nn.Module):
    ...
    def spcd_loss(self, pred, gt):
        ...
        Args:
            ...
            gt   (torch.Tensor[N,24,2]): dsnt result of the gt heatmap
        """
        inter_points = (gt[:, [19, 18, 15, 14, 11, 10, 7, 6, 3, 2]] -
                        pred[:, [19, 18, 15, 14, 11, 10, 7, 6, 3, 2]])  # torch.Size([8, 10, 2])
        loss = ((inter_points[:, 1::2]-inter_points[:, ::2])
                ** 2).sum(dim=2).mean(dim=1)  # torch.Size([8])
